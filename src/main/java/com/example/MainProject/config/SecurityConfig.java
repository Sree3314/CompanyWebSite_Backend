 ...
 * Spring Security Configuration for the unified MainProject application.
 *
 * THIS VERSION INCLUDES ALL RULES FOR AUTH, UPLOADS, EXHIBITION, AND EMAIL
 * PASSWORD RESET. CORRECTED: `addFilterBefore` usage.
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(AbstractHttpConfigurer::disable).cors(cors -> cors.configurationSource(corsConfigurationSource()))
				...
				.authorizeHttpRequests(authorize -> authorize
						...
						.requestMatchers(HttpMethod.GET, "/api/jobs/manager/**").hasRole("MANAGER")
						.requestMatchers(HttpMethod.GET, "/api/jobs").hasAnyRole("MANAGER", "USER")
						.requestMatchers(HttpMethod.DELETE, "/api/jobs/**").hasRole("MANAGER")
		...
		http.authenticationProvider(authenticationProvider());
		// CORRECTED LINE: jwtRequestFilter runs before
