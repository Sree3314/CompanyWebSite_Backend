...
//        // Sort the list in memory by uploadDate in descending order (latest first)

//        Collections.sort(showcaseUploads, Comparator.comparing(Upload::getUploadDate).reversed());

        //System.out.println("Backend Service: >>> Fetching ALL showcase items, attempting in-memory sort (latest at top). <<<");
 
        // Fetch all approved showcase uploads using the existing repository method

        List<Upload> showcaseUploads = uploadRepository.findByIsForShowcaseTrueAndApprovalStatus("approved");
 
//        System.out.println("Backend Service: --- showcaseUploads BEFORE sorting (Upload ID & Dates): ---");

//        for (Upload upload : showcaseUploads) {

//            System.out.println("  - Upload ID: " + upload.getId() + ", Date: " + upload.getUploadDate());

//        }
 
        // CRITICAL: This line MUST be uncommented and correctly applied

        Collections.sort(showcaseUploads, Comparator.comparing(Upload::getUploadDate).reversed());
 
//        System.out.println("Backend Service: --- showcaseUploads AFTER sorting (Upload ID & Dates): ---");
...
//        System.out.println("Backend Service: >>> Sorting attempt complete. <<<");
    ...
    public List<ShowcaseItemDTO> getFilteredShowcaseItems(Long externalEmployeeId, String firstName) {
     ...
     // IMPORTANT CHANGE: Add order by uploadDate DESCENDING

        cq.orderBy(cb.desc(root.get("uploadDate"))); // Sort by uploadDate in descending order
